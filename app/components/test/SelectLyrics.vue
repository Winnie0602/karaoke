...
<template>
  <div
    class="hide-scroll flex h-[calc(100vh-56px-32px-64px-40px-20px)] flex-col rounded-md border-[1px] border-gray-300 bg-white md:block"
  >
    <div class="sticky top-0 z-20 flex h-[30px] items-center bg-[#e6ebf1] pl-3">
    ...
    <div
      class="flex h-full w-full flex-col overflow-hidden md:h-[calc(100%-30px)] md:flex-row"
    >
      <div
        class="hide-scroll z-10 flex shrink-0 flex-row items-center space-y-0 space-x-2 overflow-x-auto bg-[#f6f6f6]/50 px-2 py-2 md:flex-[1] md:flex-col md:items-stretch md:space-y-1 md:space-x-0 md:overflow-y-hidden md:px-3 md:py-4"
      >
        <div
      ...
      >
        <div
          ...
          :key="lyric.start"
          class="group relative flex flex-col justify-center px-4 py-3 text-sm transition-all md:flex-row md:items-center md:justify-between md:px-3 md:py-2 md:text-base"
          :class="{
          ...
          <div class="flex flex-col pr-12 md:pr-0">
            <div class="flex flex-wrap">
              <span
                ...
                :key="wIndex"
                class="mr-1 inline-block font-medium"
              >
                {{ word.surface }}
              </span>
            </div>
